...
class RtdmEventAction(Action):
    ...
    def __init__(self, items: Dict[str, Any], id: Optional[str] = None):
        ...
        self.topic: str = items.get("topic", "rtdm")
        self.request_data = items.get("request_data", self.DEFAULT_REQUEST_DATA)
        self.request_data[self.EX_HEADERS_NAME] = self.request_data.get(self.EX_HEADERS_NAME,
                                                                        self.DEFAULT_EXTRA_HEADERS)
        self.notification_code = items.get("notification_code")
        self.producer = KafkaPublisher
        # Есть ли необходимость в строчках ниже? Макар не видит.
        items["request_data"] = self.request_data
        items["command"] = RTDM
    ...
    async def run(self, user: BaseUser, text_preprocessing_result: BaseTextPreprocessingResult,
        ...
        if offer_or_service:  # если оффер или сервис был получен
            ...
            await self.producer.send_and_wait(topic=self.topic,
                                              key=transport_message["key"],
                                              value=transport_message["body"].to_bytes(),
                                              headers=self.request_data[self.EX_HEADERS_NAME])
