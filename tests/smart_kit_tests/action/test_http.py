...
from smart_kit.action.http import HTTPRequestAction, AsyncHTTPRequestAction
from tests.smart_kit_tests.action.test_base_http_action import BaseHttpRequestActionTest, AsyncBaseHttpRequestActionTest
...
class AsyncHttpRequestActionTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.user = Mock(
            parametrizer=Mock(collect=lambda *args, **kwargs: {}),
            descriptions={
    ...
    async def test_simple_request(self, request_mock: Mock):
        AsyncBaseHttpRequestActionTest.set_request_mock_attribute(request_mock, return_value={'data': 'value'})
        items = {
            "params": {
                "method": "POST",
                "url": "https://my.url.com",
        ...
        await AsyncHTTPRequestAction(items).run(self.user, None, {})
        request_mock.assert_called_with(url="https://my.url.com", method='POST', timeout=3)
        self.assertTrue(self.user.descriptions["behaviors"]["my_behavior"].success_action.run.called)
        self.assertTrue(self.user.variables.set.called)
        self.user.variables.set.assert_called_with("user_variable", {'data': 'value'})
